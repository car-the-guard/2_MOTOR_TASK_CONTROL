...
static uint8_t g_tx_counter = 0; // [추가] 송신용 카운터
...
// [수정] 보안 검증이 적용된 패킷 처리 함수
void Process_Packet(CAN_RxPacket_t *pkt) {
    uint8_t *pData = pkt->data;
    uint8_t rx_cnt = pData[6]; // 카운터 (Byte 6)
    uint8_t rx_mac = pData[7]; // MAC (Byte 7)

    // -----------------------------------------------------
    // 1. MAC 무결성 검증 (공통)
    // -----------------------------------------------------
    // 패킷 데이터(0~6)와 수신된 MAC(7)이 일치하는지 확인
    if (verify_mac(pData, 7, rx_cnt, rx_mac) == 0) {
        #if (DEBUG_MODE == 1)
        // mcu_printf("[SEC] MAC Verification Failed! ID:0x%X\n", pkt->id);
        #endif
        return; // [중요] 검증 실패 시 즉시 리턴 (명령 무시)
    }
    ...
    if (pkt->id == CAN_ID_AEB) {
        // [Replay Attack 방지] 이전 카운터보다 커야 함 (Wrap-around 예외 처리)
        if (g_last_aeb_counter != -1 && rx_cnt <= g_last_aeb_counter) {
            if (!((g_last_aeb_counter > 240) && (rx_cnt < 15))) { // 255->0 넘어가는 상황이 아니면
                 return; // 오래된 패킷 무시
            }
        }
        g_last_aeb_counter = rx_cnt; // 카운터 갱신

        // [기존 AEB 로직]
        if (pData[0] != 0x00) {
            if(!g_aeb_active) {
                mcu_printf("!!! [RX] AEB ACTIVE (SECURED) !!!\n");
                g_aeb_active = 1;
                Emergency_Stop_Hard(); 
            }
        } else {
            if(g_aeb_active) mcu_printf("... [RX] AEB RELEASED (SECURED) ...\n");
            g_aeb_active = 0;
    ...
    // (B) 모터 제어 명령
        ...
        if (g_aeb_active) return; // AEB 중엔 제어 무시

        // [Replay Attack 방지]
        if (g_last_cmd_counter != -1 && rx_cnt <= g_last_cmd_counter) {
             if (!((g_last_cmd_counter > 240) && (rx_cnt < 15))) {
                 return; // 오래된 패킷 무시
            }
        }
        g_last_cmd_counter = rx_cnt; // 카운터 갱신
...
// [수정] 송신 데이터에도 MAC과 Counter를 포함
static void CAN_Send_Data_With_Time(uint32 id, int32 value_scaled, uint16 timestamp) {
    CANMessage_t txMsg; 
    uint8 ucTxBufferIndex;
    uint8_t payload[8]; // 임시 버퍼
    ...
    // 1. 데이터 채우기
    payload[0] = (uint8)((value_scaled >> 24) & 0xFF);
    payload[1] = (uint8)((value_scaled >> 16) & 0xFF);
    payload[2] = (uint8)((value_scaled >> 8) & 0xFF);
    payload[3] = (uint8)(value_scaled & 0xFF);
    payload[4] = (uint8)((timestamp >> 8) & 0xFF);
    payload[5] = (uint8)(timestamp & 0xFF);
    payload[6] = g_tx_counter; // [추가] 카운터

    // 2. MAC 계산 (payload[0]~payload[6]까지 7바이트로 계산)
    payload[7] = compute_mac(payload, 7, g_tx_counter); // [추가] MAC 생성
